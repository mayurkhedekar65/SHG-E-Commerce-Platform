    ...
    return (
      <>
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-[#FDFDFD]">
          ...
          <p className="text-[#333333] font-medium tracking-widest uppercase text-sm italic">Synchronizing...</p>
        </div>
      </>
    );
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
              ...
              <AnimatePresence mode="popLayout">
                {cartItems.length === 0 ? (
                  <div className="bg-white p-20 rounded-3xl shadow-sm text-center border border-gray-100 flex flex-col items-center">
                    ...
                    <p className="text-gray-500 text-xl font-light italic">Your bag is empty.</p>
                  ...
                  <div className="space-y-6">
                    {cartItems.map((item) => (
                      <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} key={item.id} className="group bg-white p-6 rounded-[2.5rem] shadow-sm flex flex-col sm:flex-row items-center gap-8 border border-gray-50 overflow-hidden">
                        ...
                        <div className="flex-1 w-full">
                          <div className="flex justify-between items-start mb-2">
                            <h3 className="font-bold text-2xl uppercase tracking-tight">{item.name || "Untitled"}</h3>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-6 bg-[#F9F9F9] px-6 py-2 rounded-xl border border-gray-100">
                              <button onClick={() => updateQuantity(item.id, item.quantity, -1)} className="text-gray-400 hover:text-[#333333]">
                                <FontAwesomeIcon icon={faMinus} size="sm" />
                              </button>
                              <span className="font-black text-lg w-4 text-center">{item.quantity}</span>
                              <button onClick={() => updateQuantity(item.id, item.quantity, 1)} className="text-gray-400 hover:text-[#F5C469]">
            ...
            <div className="lg:col-span-4">
              <div className="bg-[#333333] text-white p-10 rounded-[2.5rem] shadow-2xl sticky top-36 border border-[#444444]">
                <h2 className="text-3xl font-black mb-8 italic uppercase underline decoration-[#F5C469] decoration-4 underline-offset-8">Order <span className="text-[#F5C469]">Details</span></h2>

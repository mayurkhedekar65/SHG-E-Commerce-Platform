...
const Home = () => {
  ...
  const [shgGroups, setShgGroups] = useState([]); // Database groups state
  const [loading, setLoading] = useState(false);
  const [fetchLoader, setFetchLoader] = useState(true);
  const navigate = useNavigate();

  // 2. Authentication Popup States
  ...
  // 4. Fetch Dynamic Data from Django Backend
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch Featured Products
        const productResponse = await axios.get("http://127.0.0.1:8000/get_products/");
        setProducts(productResponse.data["products_list"] || []);

        // Fetch Featured SHG Groups
        const groupResponse = await axios.get("http://127.0.0.1:8000/shg_groups/");
  ...
  // 5. Auth Success Handler (Full Page Refresh)
      ...
      {/* {loading ? (
        ...
        <>
            ...
            <section className="text-center py-14 bg-[#dddddd]">
              <div>
                ...
                <p className="text-[#333333] md:text-[17px] mt-4">
                  Explore authentic Goan products crafted with love by local Self Help Groups
                </p>
              </div>
              <div className="flex flex-col md:flex-row gap-5 justify-center items-center md:gap-12 text-center pt-10">
                <ShopCategoryCard Logo={faUtensils} CategoryType={"Food & Beverages"} />
          ...
          >
                  ...
                  {products.length > 0 ? (
                    ...
                    <>
                      <ProductCard
                        ...
                        Amount={"350"}
                        Quantity={500}
                        Description={"Cold-pressed coconut oil made by local women's SHG."}
                        Category={"Edible Oil"}
                        onRequireLogin={() => setShowSigninPopup(true)}
                      />
                      <ProductCard
                        ...
                        Amount={"450"}
                        Quantity={200}
                        Description={"Pure organic honey harvested by the Sunshine Women's SHG."}
                      ...
                      <ProductCard
                        ...
                        Amount={"150"}
                        Quantity={300}
                        Description={"Natural handmade soap with soothing lavender fragrance."}
                      ...
                      <ProductCard
                        ...
                        Amount={"600"}
                        Quantity={150}
                        Description={"Eco-friendly handwoven cotton bag by Mountain Weavers SHG."}
            ...
            {/* Featured SHG Groups Section (Dynamically Linked) */}
            <section className="text-center py-14 bg-[#dddddd]">
              <div>
                <h2 className="font-bold text-[#333333] font-sans text-2xl md:text-3xl">
                  Featured SHG Groups
                ...
                <p className="text-[#333333] md:text-[17px] mt-4">
                  Meet the amazing Self Help Groups creating beautiful products across Goa
                </p>
              </div>
                  ...
                  {shgGroups.length > 0 ? (
                    ...
                    <>
                      <GroupCard
                        ...
                        Address={"Plot No. 7, Hillview Street, Chorla, Sattari, North Goa"}
                      />
                      <GroupCard
                        GroupImage1={shgimg2}
                        ...
                        Address={"Shop 5, Beach Road, Calangute Village, Bardez, North Goa"}
                      />
                      <GroupCard
                        GroupImage1={shgimg3}
                      ...
                      <GroupCard
                        ...
                        Address={"Plot No. 24, Main Street, Kudal Village, Sindhudurg, Maharashtra"}
                      />
                    </>
                  )}
                </div>
            ...
            <section className="text-center py-14 bg-[#333333]">
              <div>
                ...
                <p className="text-[#dddddd] md:text-[17px] mt-4">
                  Supporting local communities and preserving traditional crafts across Goa
                </p>
              </div>
              <div className="flex flex-col md:flex-row justify-center items-center gap-5 md:gap-10 text-center pt-10">
                <ImpactCard TotalNumber={"150"} CardText={"SHG Groups"} Logo={faUser} />
                <ImpactCard TotalNumber={"2500"} CardText={"Products"} Logo={faBox} />
                <ImpactCard TotalNumber={"10,000"} CardText={"Happy Customers"} Logo={faHeart} />
                <ImpactCard TotalNumber={"50L"} CardText={"Revenue Generated"} Logo={faRupeeSign} />
              </div>
            </section>
            ...
            <section className="text-center py-14 bg-[#dddddd]">
              ...
              <div className="flex flex-col md:flex-row justify-center items-center gap-8 md:gap-10 text-center pt-10">
                <ProcessCard
                  ...
                  Description={"Explore our curated collection of Goan products from verified SHGs"}
                />
                <ProcessCard
                  Number={"2"}
                  Title={"Connect with SHGs"}
                  Description={"View seller details and communicate your requirements through our platform"}
                />
                <ProcessCard
                  Number={"3"}
                  Title={"Secure Checkout"}
                  Description={"Place your order using our secure gateway or choose cash on delivery"}
                />
                <ProcessCard
                  Number={"4"}
                  Title={"Receive & Enjoy"}
                  Description={"Get your authentic Goan products delivered safely to your doorstep"}
                />
              </div>
            </section>
          </motion.div>
